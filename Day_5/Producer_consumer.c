...
void *producer(void *arg){
	...
	while(1)
	{
		while(counter ==5);// for identification buffer is full 
		buffer[i]=ch;
		i = (i+1)%5;// getting i and increment 
		printf("%d\n",i);
                sleep(1);
...
void* consumer(void *arg){
	...
	char data[5];
        ...
        while(1)
        {
		while(counter ==0);//for identifiction buffer is empty
		data[out]=buffer[out];
		out = (out+1)%5;// out value udation
